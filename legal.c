...
/*conv_bm_cases(bm_pos, &row, &col);	//non serve...
...
int is_illegal(int order, piece *friends, piece *enem) {	//looks for an enemy piece eating king
	char en_type, fr_type;
	char enem_color;
	int j;
	BITMASK king_pos;
	int k_row, k_col;
...
int is_legal_mov(int order, piece *friends, piece *enem, BITMASK bm_mov) {      //we test legality for friends[order]
	int j, eaten, legal;
